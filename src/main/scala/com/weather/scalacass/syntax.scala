...
object syntax {
  implicit class RichRow(private val r: Row) extends AnyVal {
    ...
    def getOrElse[T](name: String, default: => T)(implicit d: CassFormatDecoder[Option[T]]): T =
      d.as(r)(name).getOrElse(default)
  ...
  implicit class RichIterator(private val it: Iterator[Row]) extends AnyVal {
    def as[T](name: String)(implicit d: CassFormatDecoder[T]): Iterator[T] = it.map(r => d.as(r)(name))
    def getAs[T](name: String)(implicit d: CassFormatDecoder[Option[T]]): Iterator[Option[T]] =
      it.map(r => d.as(r)(name))
    def getOrElse[T](name: String, default: => T)(implicit d: CassFormatDecoder[Option[T]]): Iterator[T] =
      it.map(r => d.as(r)(name).getOrElse(default))
    def attemptAs[T](name: String)(implicit d: CassFormatDecoder[T]): Iterator[Result[T]] =
      it.map(r => d.attemptAs(r)(name))
    ...
    def getOrElse[T](default: => T)(implicit ccd: CCCassFormatDecoder[Option[T]]): Iterator[T] =
      it.map(r => ccd.as(r).getOrElse(default))
    def attemptAs[T](implicit ccd: CCCassFormatDecoder[T]): Iterator[Result[T]] = it.map(r => ccd.attemptAs(r))
  }

  implicit class RichOption(private val opt: Option[Row]) extends AnyVal {
    def as[T](name: String)(implicit d: CassFormatDecoder[T]): Option[T] = opt.map(r => d.as(r)(name))
    def getAs[T](name: String)(implicit d: CassFormatDecoder[Option[T]]): Option[Option[T]] =
      opt.map(r => d.as(r)(name))
    def getOrElse[T](name: String, default: => T)(implicit d: CassFormatDecoder[Option[T]]): Option[T] =
      opt.map(r => d.as(r)(name).getOrElse(default))
    def attemptAs[T](name: String)(implicit d: CassFormatDecoder[T]): Option[Result[T]] =
      opt.map(r => d.attemptAs(r)(name))
    ...
    def getOrElse[T](default: => T)(implicit ccd: CCCassFormatDecoder[Option[T]]): Option[T] =
      opt.map(r => ccd.as(r).getOrElse(default))
