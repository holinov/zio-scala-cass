...
abstract class CassandraWithTableTester(
    val dbName: String,
    protected val tableName: String,
    tableColumns: List[String],
    primaryKeys: List[String]
) extends CassandraUnitTester {
  override def beforeAll(): Unit = {
    super.beforeAll()
    client.session.execute(
      s"CREATE KEYSPACE $dbName WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 1};"
    )
    client.session.execute(s"CREATE TABLE $dbName.$tableName ${tableColumns
      .mkString("(", ", ", ",")} PRIMARY KEY ${primaryKeys.mkString("((", ", ", "))")})")
    ()
  }
  ...
  protected def insert(pairs: Seq[(String, AnyRef)]) = {
    ...
    client.session.execute(
      s"INSERT INTO $dbName.$tableName ${strs.mkString("(", ",", ")")} VALUES ${objs.map(_ => "?").mkString("(", ",", ")")}",
      objs: _*
    )
  }
